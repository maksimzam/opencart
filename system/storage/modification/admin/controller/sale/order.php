...
class ControllerSaleOrder extends Controller {
	...
	public function delete() {
		...
		$this->getList();
            ...
            public function save_to_excel() {
                ...
                if (count($results) > 0) {
                  include (DIR_SYSTEM. 'PHPExcel/Classes/PHPExcel.php');
                  $objPHPExcel = new PHPExcel(); // Create new PHPExcel object
                  $objPHPExcel->getProperties()->setCreator("OpenCart")
                    ->setLastModifiedBy("OpenCart")
                    ->setTitle("OpenCart - order")
                    ->setSubject("OpenCart - order")
                    ->setDescription("")
                    ->setKeywords("")
                    ->setCategory("OpenCart");
                  // create style
                    ...
                    $objPHPExcel->setActiveSheetIndex(0)
                            ->setCellValue('A1', $this->language->get('entry_excel_order_id'))
                            ->setCellValue('B1', $this->language->get('entry_excel_added_date'))
                            ->setCellValue('C1', $this->language->get('entry_excel_name'))
                            ->setCellValue('D1', $this->language->get('entry_excel_address'))
                            ->setCellValue('E1', $this->language->get('entry_excel_products'))
                            ->setCellValue('F1', $this->language->get('entry_excel_summ'));


                    $objPHPExcel->getActiveSheet()->getStyle('A1:F1')->applyFromArray( $style_header ); // give style to header
                    ...
                    foreach ($results as $result) {
                        ...
                        foreach ($products as $product) {
                         ...
                         $objPHPExcel->setActiveSheetIndex(0)
                                    ->setCellValue('A'.$urut, $result['order_id'])
                                    ->setCellValue('B'.$urut, $result['date_added'])
                                    ->setCellValue('C'.$urut, $result['customer'])
                                    ->setCellValue('D'.$urut, $order['shipping_address_1'] . " " .$order['shipping_address_2'])
                                    ->setCellValue('E'.$urut, $product_str)
                                    ->setCellValue('F'.$urut, $result['total']);
                         $laststyle='F'.$urut;
                       $i++;
                    }

                    $objPHPExcel->getActiveSheet()->getStyle('A2:'.$laststyle)->applyFromArray( $style_content ); // give style to header
	...
	protected function getList() {
		...
		$data['button_view'] = $this->language->get('button_view');
            ...
            $data['entry_load_to_excel'] = $this->language->get('entry_load_to_excel');
            $data['error_select_date'] = $this->language->get('error_select_date');
		...
		$data['filter_date_added'] = $filter_date_added;
            ...
            $data['filter_date_added_start'] = date("Y-m-d",mktime(0,0,0,date("m"),1,date("y")));
